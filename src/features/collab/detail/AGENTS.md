...
- Collab diffs are intentionally text-only. `review/CollabDiffRenderer.ts` must pass `lang: text` and select Shiki's JavaScript engine; `review/CollabShikiAdapter.ts` retains the real Shiki core with no bundled grammars, and `review/CollabPierreThemes.ts` registers only `pierre-dark` and `pierre-light`. The guarded build replacement owns Pierre's exact `shiki`, theme-catalog, and optional Wasm imports. Keep `@pierre/diffs` pinned to the verified dependency contract. A Pierre upgrade must first update the contract, dependency-envelope, and real-DOM text-render tests.
